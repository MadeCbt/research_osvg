...
class GameInterface:
    ...
    def run_quick_test(self):
        ...
        with self.db.engine.connect() as conn:
            ...
            conn.execute(text("DELETE FROM video_games WHERE _id = :id"), {"id": test_game_id})
            # Delete test author
            conn.execute(text("DELETE FROM authors WHERE _id = :id"), {"id": test_author_id})
            conn.commit()
    ...
    def delete_game(self):
        ...
        try:
            ...
            # Confirm deletion
            confirm = input(f"⚠️ Delete '{game_to_delete.title}'? This cannot be undone! (y/n): ").strip().lower()
            if confirm != 'y':
    ...
    def delete_developer(self):
        ...
        try:
            ...
            if delete_choice == "1":
                ...
                confirm = input("⚠️ This will delete ALL games by this developer! Continue? (y/n): ").strip().lower()
                if confirm == 'y':
                    if self.db.delete_author_and_games(author_to_delete._id):
                        print(f"✅ Deleted developer '{author_to_delete.name}' and all their games")
                    else:
